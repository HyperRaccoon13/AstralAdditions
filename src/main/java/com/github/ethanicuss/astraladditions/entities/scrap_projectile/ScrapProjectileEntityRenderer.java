...
public class ScrapProjectileEntityRenderer
        extends EntityRenderer<ScrapProjectileEntity> {
    private static final Identifier TEXTURE = new Identifier(AstralAdditions.MOD_ID, "textures/entity/scrap_projectile/scrap_projectile.png");
    private static RenderLayer LAYER = RenderLayer.getEntityCutoutNoCull(TEXTURE);

    public ScrapProjectileEntityRenderer(EntityRendererFactory.Context context) {
        super(context);
    }

    @Override
    protected int getBlockLight(ScrapProjectileEntity scrapEntity, BlockPos blockPos) {
        return 15;
    }

    @Override
    public void render(ScrapProjectileEntity scrapEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        matrixStack.push();
        ...
        matrixStack.pop();
        super.render(scrapEntity, f, g, matrixStack, vertexConsumerProvider, i);
    }
    ...
    @Override
    public Identifier getTexture(ScrapProjectileEntity cometballEntity) {
